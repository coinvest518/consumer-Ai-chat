...
export const getChatHistoryCollection = async () => {
    try {
...
export const getUserMetricsCollection = async () => {
    try {
...
export const storeChatMessage = async (userId, sessionId, message) => {
    try {
...
export const updateUserMetrics = async (userId, metrics) => {
    try {
        ...
        if (existingMetrics) {
            ...
            await collection.updateOne({ userId }, { $set: { ...metrics, updatedAt: now } });
            return true;
        }
        else {
